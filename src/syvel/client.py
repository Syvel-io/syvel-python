...
class Syvel:
    ...
    Wraps the Syvel API using ``httpx.Client`` for connection pooling.
    Can be used as a context manager to ensure the underlying HTTP client
    is properly closed:
    ...
    def check(self, domain: str) -> CheckResult | None:
        ...
        """
        data = self._request("GET", f"/v1/check/{_encode_target(domain)}")
        if data is None:
            return None
        return CheckResult.from_dict(data)

    def check_email(self, email: str) -> CheckResult | None:
    ...
    def usage(self) -> UsageResult | None:
    ...
    def stats(self) -> list[StatsPoint] | None:
    ...
    def list_keys(self) -> list[ApiKey] | None:
    ...
    def create_key(self, name: str) -> ApiKey | None:
        ...
        Returns:
            ...
            and an error occurred.
        """
        data = self._request("POST", "/v1/keys", json={"label": name})
        if data is None:
            return None
